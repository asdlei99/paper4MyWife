...
if 1
    %单容
    dataIndex = 12;
    simDataPath = fullfile(dataPath,'实验原始数据\罐二在弯头处\双缓冲罐串联罐二当弯头420转0.1mpa');
    ...
    x = 0:1:(size(res,1)-1);
    x = x .* detalTime;
    legendText = {'560,265','1,140,145','2,216,754'};
    figure
    set(gcf,'color','w');
    ...
    if 0
        set(gca,'color','none');
        saveFigure(fullfile(getPlotOutputPath(),'ch07'),'双容串联-网格无关性验证图');
    end
end
