...
if 1
    ...
    belowPressure = doubleVesselBeElbowFrequencyResponse('param',param,'fs',fs);
    doubleVesselPressure = doubleVesselFrequencyResponse('param',param,'fs',fs);
    
    X= [param.sectionL1,param.L1+param.LV1+2*param.l+param.sectionL2 ,param.L1+param.LV1+2*param.l+param.L2+param.lv3+param.DV2/2+param.sectionL3];
    if isEnglish
        fontName = 'Cambria';
        barHighText = 'High';
        barLowText = 'Low';
    ...
    subplot(1,2,1)
	plotWaveWaterFall(belowPressure,fs,'y',X,'type',type);
    xlabel(xlabelText,'FontName',fontName,'FontSize',10);
    ylabel(ylabelText,'FontName',fontName,'FontSize',10);
%     xlim([0,100]);
    ax = axis();
    hold on;
    %绘制罐二做弯头的L1分界
    plot([ax(1),ax(2)],[param.L1,param.L1],'--w');
    plot([ax(1),ax(2)],[param.L1+param.LV1+2*param.l,param.L1+param.LV1+2*param.l],'--w');
    
    subplot(1,2,2)
    plotWaveWaterFall(doubleVesselPressure,fs,'y',X,'type',type);
    hold on;
%     xlim([0,100]);
    xlabel(xlabelText,'FontName',fontName,'FontSize',10);
    ylabel(ylabelText,'FontName',fontName,'FontSize',10);
