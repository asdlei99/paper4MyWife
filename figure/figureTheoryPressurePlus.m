...
while length(pp)>=2
    ...
    switch lower(prop)
        ...
        case 'fontname'
            fontName = val;
        case 'marksectionxcolor'
            markSectionXColor = val;
        case 'marksectionycolor'
            markSectionYColor = val;
        otherwise
       		error('参数错误%s',prop);
    end
end
...
if isnan(Y)
    ...
    if strcmp(chartType,'plot3')
        fh = figurePlotPlot3(dataCells,X,Y,sectionY,markSectionY...
            ,sectionX,markSectionX,markSectionXColor,markSectionYColor);
        box on;
    elseif strcmp(chartType,'surf')
        fh = figurePlotSurf(dataCells,X,Y,edgeColor...
            ...
            ,markSectionXColor,markSectionYColor...
            ,fixAxis...
            );
        box on;
        grid on;
...
function fhRet = figurePlotPlot3(dataCells,X,Y,sectionY,markSectionY,sectionX...
    ,markSectionX,markSectionXColor,markSectionYColor)
    hold on;
     for i = 1:length(dataCells)
