%% 单一缓冲罐固定管长，改变L1和L2对气流脉动的影响
function theoryDataCells = oneVesselChangL1FixL(V,varargin)
pp = varargin;
massflowData = nan;
while length(pp)>=2
    prop =pp{1};
...
for i = 1:length(L1)
    ...
    theoryDataCells{i+1,1} = sprintf('缓冲罐进口管长:%g',L1(i));
    ...
    theoryDataCells{i+1,4} = param.L1;
    theoryDataCells{i+1,5} = param.L2;
    theoryDataCells{i+1,6} = param;
