function theoryDataCells = doubleVesselBeElbowChangDistanceAndVolume(varargin)
% 双罐第二个缓冲罐作为弯头迭代双罐中间连接管长和第二罐体积
    pp = varargin;
    massflowData = nan;
...
%       长度 L1     l    Lv    l    L2   l    Lv
    ...
    theoryDataCells{1,2} = 'dataStrcutCell2D';
    theoryDataCells{1,3} = 'X-长度';
    theoryDataCells{1,4} = 'Y-体积';
    theoryDataCells{1,5} = '接管长';
    L3 = [0:0.25:4];
%     maxLLength = param.L1 + L3(end) + param.L3;
    maxLLength = 9+2.2+0.04;%28;%总管长
    for count = 1:length(L3)
        param.L2 = L3(count);
%         param.L3 = maxLLength - param.L2 - param.L1;
        param.L3 = maxLLength - param.L2 - param.L1-param.LV1-param.LV2-4*param.l;
