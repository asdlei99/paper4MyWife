...
for i = 1:length(dp)     
    ...
    para(i).vhpicStruct.Din = 0.106/2;
    para(i).vhpicStruct.la1 = para(i).vhpicStruct.Lin - para(i).vhpicStruct.lp1-para(i).vhpicStruct.la2;%0.25;%内插管入口段长度
    para(i).vhpicStruct.lb2 = para(i).vhpicStruct.Lout - para(i).vhpicStruct.lp2-para(i).vhpicStruct.lb1;%0.25;
    ...
    para(i).vhpicStruct.s2 = ((para(i).vhpicStruct.lp2./para(i).vhpicStruct.Cloum2)-para(i).vhpicStruct.dp2)./2;
...
%                                           | L3 
%                                           |
%计算双罐-罐二作弯头内插孔板腔体二内插孔管
    [pressure1DBEH,pressure2DBEH,pressure3DBEH] = ...
        doubleVesselElbowPerfInOpenOutClosePulsationCalc(massFlowE,Fre,time,...
            para(i).L1,para(i).L3,para(i).L4,...
    ...
    Fr(i) = opt.acousticVelocity./(2.*pi).*sqrt((pi.*para(i).dp^2./4)./(para(i).lc1.*(pi.*para(i).DV2.^2./4.*(para(i).LV2./2)-(pi.*(para(i).dp+0.008).^2/4).*para(i).lc1)));%0.008是管壁厚
