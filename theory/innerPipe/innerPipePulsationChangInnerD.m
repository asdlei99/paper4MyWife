...
function theoryDataCells = innerPipePulsationChangInnerD(Dinnerpipe,varargin)
pp = varargin;
...
if ~isFast
	...
	theoryDataCells{1,4} = '内插管管径';
	theoryDataCells{1,5} = 'input';
end
for i=1:length(Dinnerpipe)
	param.Dinnerpipe = Dinnerpipe(i);
	...
	if ~isFast
		...
		theoryDataCells{i+1,4} = Dinnerpipe(i);
