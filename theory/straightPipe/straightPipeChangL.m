%% 直管改变管径
function theoryDataCells = straightPipeChangL(L,varargin)
% DPipeRang 管径的变化范围
...
% DPipeRang对应的不同流速通过baseDipe和baseFlowVelocity计算
	...
	if ~fast
		...
		theoryDataCells{1,6} = '管长';
	end

	for i = 1:length(L)
		%计算流速
		param.L = L(i);
		param.sectionL = 0:0.5:param.L;
