function M = innerPerfPipeDoubOpenTestTransferMatrix(n2,dp2,Din,Dv,lp2,lc,lb1,lb2,varargin)
%对应孔管内部部分
...
% lc 孔管壁厚
    ...
    AperB = calcCoeffAperB(k,areaRatio,M1,lb2);
    CperD = calcCoeffCperD(k,areaRatio,M1,lb2);
    ...
    T07 = -(1./(1i.*k+M1.*beita(1)) + C31./(1i.*k+M1.*beita(3)) + C41./(1i.*k+M1.*beita(4)));
    T08 = -(1./(1i.*k+M1.*beita(2)) + C32./(1i.*k+M1.*beita(3)) + C42./(1i.*k+M1.*beita(4)));
    T09 = -(exp(beita(1).*lp2)./(1i.*k+M1.*beita(1)) + C31.*exp(beita(3).*lp2)./(1i.*k+M1.*beita(3)) + C41.*exp(beita(4).*lp2)./(1i.*k+M1.*beita(4)));
    T10 = -(exp(beita(2).*lp2)./(1i.*k+M1.*beita(2)) + C32.*exp(beita(3).*lp2)./(1i.*k+M1.*beita(3)) + C42.*exp(beita(4).*lp2)./(1i.*k+M1.*beita(4)));
...
function AperB = calcCoeffAperB(k,areaRatio,M1,lb2)
    AperB = k.*(cot(k.*lb2) - ((areaRatio+1).*(1-1./areaRatio)^2.*(1-exp(1i.*k.*lb2.*M1)) ...
             + 1i.*(1+areaRatio.*(1-1./areaRatio)^2).*M1.*sin(k.*lb2).*cos(k.*lb2))...
             ./(sin(k.*lb2).*((1+areaRatio.*(1-1./areaRatio)^2).*cos(k.*lb2) ...
             + 1i.*M1.*sin(k.*lb2))));
end

function CperD = calcCoeffCperD(k,areaRatio,M1,lb2)
    CperD = k.*(cot(k.*lb2) + ((1-(1-1/areaRatio)^2).*(exp(-1i.*k.*lb2.*M1)-1))...
             ./(sin(k.*lb2).*((1+areaRatio.*(1-1/areaRatio)^2).*cos(k.*lb2) + 1i.*M1.*sin(k.*lb2))));
end
