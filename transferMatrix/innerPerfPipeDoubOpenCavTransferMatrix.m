function M = innerPerfPipeDoubOpenCavTransferMatrix(n2,dp2,Din,Dv,lp2,lc,lb1,lb2,varargin)
...
[Ta2,Tb2,Tc2,Td2] = calcT1(n2,dp2,Din,Dv,lb1,lb2,lp2,k,M1,M2,R0,lc);
coeffFM = Td2*Ta2-Tb2*Tc2;



A = Td2./coeffFM;
B = (-a./(Sp))*Tb2./coeffFM; %B = (-a./(Sv-Sp))*Tb2./coeffFM;2017/5/27日之前用的
C = (((Sv-Sp)./a)*Tc2)./(-coeffFM);
D = -((Sv-Sp)./Sp).*Ta2./(-coeffFM);
M = [A,B;C,D];%[p2,m2]孔管出口端（开孔部分）所在缓冲罐内的压力=M*[p1,m1]
end
 
function [Ta2,Tb2,Tc2,Td2] = calcT1(n2,dp2,Din,Dv,lb1,lb2,lp2,k,M1,M2,R0,lc)
% n2 孔管穿孔数
% dp 孔管穿孔孔径
% Din 穿管管径
% lp2 穿孔部分长度
