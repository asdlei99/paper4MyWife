function M = innerPerfPipeDoubOpenLeftTransferMatrix(n1,dp1,Din,Dv,lp1,lc,la1,la2,varargin)
%对应孔管内部部分
...
[Ta9,Tb9,Tc9,Td9] = calcT1(n1,dp1,Din,Dv,la1,la2,lp1,k,M1,M2,R0,lc);
% coeffFM = Td9*Ta9-Tb9*Tc9;
...
function [Ta9,Tb9,Tc9,Td9] = calcT1(n1,dp1,Din,Dv,la1,la2,lp1,k,M1,M2,R0,lc)
% n1 孔管穿孔数
% dp 孔管穿孔孔径
% Din 穿管管径
% lp1 穿孔部分长度
...
% lc 孔管壁厚
    ...
    [a1,a2,a3,a4,a5,a6,a7,a8] = calcCoeffA(n1,dp1,Din,Dv,lp1,k,M1,M2,R0,lc);
    ...
    G3 = T03*T12-T04*T11;
    Ta9 = (T01*T12-T02*T11)./G3;
    Tb9 = (T02*T03-T01*T04)./G3;
    Tc9 = (T07*T12-T08*T11)./G3;
    Td9 = (T03*T08-T04*T07)./G3;
...
function [a1,a2,a3,a4,a5,a6,a7,a8] = calcCoeffA(n1,dp1,Din,Dv,lp1,k,M1,M2,R0,lc)
% n1 孔管穿孔数
% dp 孔管穿孔孔径
% Din 穿管管径
% lp1 穿孔部分长度
...
%     end  
    sig0 = calcSigma0(dp1);
    if M1 == 0
         kp = (R0+1i.*k.*(lc+sig0.*dp1))./bp1;%kp为穿孔声阻抗率
    elseif M1>0
        kp = (R0 + 2.48.*(1-bp1).*(M1.^(1.04+40.*dp1)) + 1i.*k.*(lc+(1-(1+398.34.*dp1).*((1-bp1).^1.44).*(M1.^0.72)).*sig0.*dp1))./bp1;%通过流
        %kp = (R0+0.48.*abs(k.*rp-M1)+1i.*k.*(lc+Cg.*sig0.*dp))./bp2;%掠过流
    else
        error('马赫数M1不能小于0');
    end
    %%如果孔管管径不相等，这部分需要重新考虑，后续计算也要重新考虑
